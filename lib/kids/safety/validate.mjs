...
export function validateCharacter(character, safetyPolicy, characterPolicy) {
  ...
  if (character.canonicalSourceVersion !== characterPolicy.version) {
    errors.push(`canonicalSourceVersion debe ser ${characterPolicy.version}.`);
  }

  if (!String(character.canonicalAsset ?? '').startsWith('/')) {
    errors.push('canonicalAsset debe ser una ruta pública absoluta que comience por /.');
  } else {
    const canonicalAssetPath = path.join('public', String(character.canonicalAsset).replace(/^\/+/, ''));
    if (!fs.existsSync(canonicalAssetPath)) errors.push(`No existe el activo canónico: ${canonicalAssetPath}`);
  }
  ...
  const serializedCharacter = JSON.stringify({
    characterType: character.characterType,
    visual: character.visual,
    promptTemplate: character.promptTemplate,
  }).toLocaleLowerCase('es');
  for (const term of characterPolicy.forbiddenLegacyTraits?.[character.id] ?? []) {
    if (serializedCharacter.includes(String(term).toLocaleLowerCase('es'))) {
      errors.push(`La ficha activa conserva un rasgo legado prohibido: ${term}`);
    }
  }
...
export function validateModelSheet(modelSheet, safetyPolicy) {
  ...
  if (!['draft', 'reference', 'approved', 'superseded'].includes(modelSheet.status)) {
  ...
  const requiredViews = modelSheet.sourceType === 'character-design-board'
    ? ['front', 'threeQuarter']
  ...
  if (modelSheet.sourceType === 'character-design-board') {
    for (const view of ['side', 'back']) {
      if (!modelSheet.pendingViews?.includes(view)) errors.push(`La hoja v4 debe registrar ${view} como vista pendiente.`);
    }
  }
